...
class BigCatSpriteState : public SpriteStateInterface {
public:
	...
	const static int STATE_IN_ITEM = 8;
	const static int STATE_IN_ITEM_WITH_SCORE = 9;
	...
	void Create(AvancezLib* system) {
		// Setup spritesheet
		SDL_Log("BigCatSpriteSheet::Create");

		char* spriteName = "data/bigcat_sprite.png";
		SpriteSheet* spriteSheet = system->createSpriteSheet(spriteName);
		this->spriteSheet = spriteSheet;

		// Init clip
		InitClip(0, 0, STANDING_ANIMATION_FRAME, standClip);
		InitClip(0, 0, WALKING_ANIMATION_FRAME, walkClip);
		...
		states->push_back(CreateEntityState(STATE_IN_ITEM, FRAME_NO_MOVE, inItemClip));
		states->push_back(CreateEntityState(STATE_IN_ITEM_WITH_SCORE, FRAME_NO_MOVE, inItemWithScoreClip));
...
private:
	...
	SDL_Rect inItemClip[FRAME_NO_MOVE];
	SDL_Rect inItemWithScoreClip[FRAME_NO_MOVE];
